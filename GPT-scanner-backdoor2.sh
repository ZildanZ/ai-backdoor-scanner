...
API_KEY=" "
MODEL="gpt-4o-mini" # Lebih hemat daripada gpt-4o
...
while read -r filepath; do
    ...
    snippet=$(head -n 20 "$filepath" | jq -Rs .)
    ...
    debug_file="debug_$(basename "$filepath").json"
    echo "$response" > "$debug_file"

    # Cek error dari API
    error_msg=$(echo "$response" | jq -r '.error.message // empty')
    if [[ -n "$error_msg" ]]; then
        echo "[-] $filepath -> ERROR API: $error_msg (lihat $debug_file)"
        continue
    fi
    ...
    if [[ "$result" == "backdoor" ]]; then
        ...
        echo "[?] $filepath -> tidak dapat dianalisis (lihat $debug_file)"
    fi
